// download-model 单测：URL/路径解析、跳过判定、下载落盘（fake fetch，完全离线）。
...
function tempRoot() {
  const dir = mkdtempSync(path.join(tmpdir(), 'download-model-'));
...
function writeExisting(target, content) {
...
describe('modelTargetPath', () => {
  it('解析到 <root>/models/<id>/onnx/model_quantized.onnx', () => {
    const root = 'E:/repo';
    expect(modelTargetPath(root)).toBe(
      path.join(
        'E:/repo',
        'models',
        'paraphrase-multilingual-MiniLM-L12-v2',
...
describe('needsDownload', () => {
  it('目标不存在：需要下载', () => {
    const root = tempRoot();
    expect(needsDownload(modelTargetPath(root))).toBe(true);
  });
...
describe('downloadModel', () => {
  it('成功下载：建目录、落盘内容、原子改名（无 .tmp 残留）、返回字节数', async () => {
    ...
    const logs = [];
    const result = await downloadModel(root, { fetchImpl: impl, log: (m) => logs.push(m) });
    const target = modelTargetPath(root);
  ...
  it('已存在且未强制：跳过（不发起请求）', async () => {
    ...
    const result = await downloadModel(root, { fetchImpl: impl });
  ...
  it('force：覆盖已存在的文件', async () => {
    ...
    await downloadModel(root, { force: true, fetchImpl: impl });
  ...
  it('HTTP 错误：抛错且不留下目标文件或 .tmp 残留', async () => {
    ...
    await expect(downloadModel(root, { fetchImpl: impl })).rejects.toThrow(/HTTP 404/);
    expect(existsSync(modelTargetPath(root))).toBe(false);
    expect(existsSync(`${modelTargetPath(root)}.tmp`)).toBe(false);
  });
});
