---
name: agent-workflow
description: 仓库内的任何变更都触发：需求、缺陷、重构、测试、文档、配置等。涵盖 worktree 创建、需求分析、checklist、执行、验收 PR 与清理的完整工作流。
whenToUse: 仓库内的任何变更都触发：需求、缺陷、重构、测试、文档、配置等
---

# agent-workflow

接到任务后，按以下步骤执行，不要跳过、合并或随意更改顺序。
...
- **先完整输出，后确认**：将整理后的完整需求分析一次性全文输出，输出完毕后再请用户确认；禁止在输出前或输出中途向用户征求确认
- 用户提出变更后，必须重新输出**完整的**需求分析全文，再请用户确认；禁止只输出增量或 diff
- 确认后的完整需求文本作为后续步骤的唯一依据。
...
- 构造两级 checklist：
  - 一级 **模块**：提交的最小单位；
  - 二级 **功能点**：可并行执行的最小单位。
- 每个**功能点**必须先变更测试再实现功能：先新增或修改 `tests/` 下的测试用例（此时应失败），再编写功能代码使其通过。
- **先完整输出，后确认**：checklist 构造完成后一次性全文输出，输出完毕后再请用户**二次确认**；禁止在输出前或输出中途向用户征求确认
- 用户提出变更后，必须重新输出**完整的** checklist 全文，再请用户确认；禁止只输出增量或 diff
- 确认后：
  - 每个**模块**写为一个 task（todo_write），一个模块对应一个 commit；
  - 模块内的**功能点可以并行**执行（可交给子代理/subagent 并行完成），每个功能点都先写测试再写实现。

## 4. 验收

全部模块完成且自测通过（测试 / format / lint）后，在本地运行完整检查：

```sh
pnpm check   # typecheck + lint + test + build，必须全部通过
```

`pnpm check` 通过后才能 push 分支并创建 PR：
