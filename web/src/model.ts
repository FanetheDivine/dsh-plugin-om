 ...
 * - 会话为纯多 step（工具循环）消息：每个 step 模型输出 thinking、text 与工具调用参数，
 *   工具结果随后写回
 * - 固定前缀 = 系统提示词 S + dsh 注入消息 D（AGENTS.md、skill 定义等，以系统消息注入），
 *   每个 step 请求重复发送；按前缀缓存计费：首轮缓存创建完整前缀，
 *   此后每个 step 缓存读取上一轮 prompt、缓存创建本轮新增量
 * - step 平均输出（stepOutputTokens：thinking、text、tool-args）只输出：按补全价计费，
 *   不进主会话上下文与缓存、不计入未压缩对话量；
 *   开启压缩 thinking 时，观察压缩把其中 thinking 部分随被压缩消息一并作为 OM 摘要输入
 * - step 平均输入（stepInputTokens：text、用户或系统消息、tool-result）只写入：
 *   计入未压缩对话量并驱动上下文增长，
 *   每个 step 新增 stepInputTokens 是原始对话量增长的唯一来源，本身不计补全价
 * - om 关闭：上下文每个 step 按 stepInputTokens 增长，
 *   n = (T − S − D) / stepInputTokens 轮后原始会话达到 T
 * - om 开启：pre-step 先反思后观察；净压力 ≈ 未压缩 step 输入量 + 未遮蔽注入量
 *   （step 输出不占上下文、不进净压力），≥ 观察阈值触发观察压缩
 *   （被压缩消息 = 未压缩 step 输入量，开启压缩 thinking 时含这些 step 的 thinking，
 *   旧块保留、未压缩量清零）；
...
export type UsageBuckets = {
  /** 补全 tokens（含每 step 输出与摘要调用 output）。 */
  completion: number;
  /** 缓存读取 tokens。 */
  cacheRead: number;
...
export type ModelParams = {
  /** 系统提示词 tokens（S）：每个 step 重复发送的固定前缀之一。 */
  ...
  /** step 平均输出 tokens（每个 step 模型输出的 thinking、text、tool-args）：只计补全价，不进主会话上下文与缓存。 */
  stepOutputTokens: number;
  /** step 平均输入 tokens（每个 step 输入给模型的 text、用户或系统消息、tool-result）：只写入缓存并计入原始对话量（驱动轮数与观察），不计补全。 */
...
/** 默认参数：阈值取插件当前默认值，step 平均输出 780 / step 平均输入 660 取实测平均值，价格取 Opus 定价。 */
...
/** 会话总轮数：原始会话规模 T 达到 (S + D + n·stepInput) 时的 n（不足一轮按 0 计；
 *  stepInput ≤ 0 视为会话不增长，恒为 0；step 输出只计补全、不影响轮数）。 */
export function turnCount(params: Readonly<ModelParams>, targetTokens: number): number {
  ...
  if (conversation <= 0 || params.stepInputTokens <= 0) return 0;
  return Math.ceil(conversation / params.stepInputTokens);
}

/**
 * om 关闭的全会话模拟：逐 step 累计三类 token。
 * 每个 step 主请求 prompt = 固定前缀 + 已写回的 step 输入累计，首轮整段缓存创建；
 * 此后每个 step 缓存读取上一轮 prompt、缓存创建本轮新增 stepInputTokens；
 * 每个 step 补全 stepOutputTokens（输出不入缓存）。
 */
export function simulateWithoutOm(
  ...
  const stepOutput = params.stepOutputTokens;
  const stepInput = params.stepInputTokens;
  ...
  let peak = 0;
  for (let t = 1; t <= n; t += 1) {
    const prompt = prefix + (t - 1) * stepInput;
    if (prevPrompt === null) {
      usage.cacheWrite += prompt;
    } else {
      usage.cacheRead += prevPrompt;
      usage.cacheWrite += stepInput;
    }
    usage.completion += stepOutput;
...
/**
 * om 开启的全会话模拟：每个 step 先 pre-step（反思 → 观察）再发主请求；
 * 每个 step 补全 stepOutputTokens（只输出、不入缓存），每个 step 的 stepInputTokens 写回未压缩量并计入缓存创建。
 ...
 * - 压缩 step 之外：主请求缓存读取上一轮 prompt、缓存创建本轮 step 输入
 */
export function simulateWithOm(
  ...
  let observeCount = 0;
  let reflectCount = 0;
  let uncompressed = 0; // 未压缩 step 输入量 W（step 输出不占上下文、不进净压力）
  ...
  let peak = 0;
  for (let t = 1; t <= n; t += 1) {
    // pre-step：压缩发生时该 step 主请求的缓存读取被替换点截断（null 表示未压缩）
    ...
    if (netPressure >= params.observeThresholdTokens) {
      ...
      // 被压缩消息 = 未压缩 step 输入（开启压缩 thinking 时含 thinking）
    ...
    peak = Math.max(peak, prompt);
    prevPrompt = prompt;
    // 本 step 的 step 输入写回未压缩量；step 输出只计补全、不占上下文，但累计供观察时一并输入 OM
    uncompressed += stepInput;
    uncompressedThinking += stepOutput;
  }
